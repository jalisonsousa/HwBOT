...
cleanersMod.procedure("createButtons", function(shared, hooks){
	...
	if(path === '/internet'){
		...
		if(logareas.length > 0){
			getDOMElement("ul", "class", "nav nav-tabs", 0).innerHTML += '<button id="removeClues" class="btn btn-success" style="padding: 7.5px 12px;"">' + LANG.EASY_BUTTON_CLEAN_TARGET_IPS + '</button>';
	...
	if(path === '/log'){
		const logareas = document.getElementsByClassName('logarea')
		if(logareas.length > 0){
			getDOMElement("ul", "class", "nav nav-tabs", 0).innerHTML += '<button id="cleanLogs" class="btn btn-success" style="padding: 7.5px 12px;"">' + LANG.EASY_BUTTON_CLEAN_MYLOGS + '</button>';
